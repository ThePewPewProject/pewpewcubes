 ...
 */
public class RenderTimerSettingCommand {
    @SuppressWarnings("SameReturnValue")
    private static int execute(CommandContext<ServerCommandSource> context) {
        var value = BoolArgumentType.getBool(context, "value");
        LasertagSettingsManager.set(context.getSource().getServer(), SettingNames.RENDER_TIMER, value);
    ...
    static void register(LiteralArgumentBuilder<ServerCommandSource> lab) {
        lab.then(literal("renderTimer")
                .then(argument("value", bool())
                        .executes(RenderTimerSettingCommand::execute)));
    }
}
