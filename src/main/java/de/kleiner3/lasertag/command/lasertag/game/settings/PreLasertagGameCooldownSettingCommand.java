 ...
 */
public class PreLasertagGameCooldownSettingCommand extends ServerFeedbackCommand {
    protected Optional<CommandFeedback> execute(CommandContext<ServerCommandSource> context) {
        ...
        return Optional.of(new CommandFeedback(Text.literal("Lasertag setting preGameCooldown is now set to " + value), false, true));
    }

    static void register(LiteralArgumentBuilder<ServerCommandSource> lab) {
        lab.then(literal("preGameCountdownDuration")
                .then(argument("duration", longArg(0))
                        .executes(new PreLasertagGameCooldownSettingCommand())));
    }
}
