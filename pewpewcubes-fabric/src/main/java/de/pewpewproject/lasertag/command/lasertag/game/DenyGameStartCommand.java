...
/**
 * Command to deny a player the ability to start a game
 *
 * @author Étienne Muser
 */
public class DenyGameStartCommand extends ServerFeedbackCommand {
    ...
    protected CompletableFuture<Optional<CommandFeedback>> execute(CommandContext<ServerCommandSource> context) {
        ...
        players.forEach(p -> {
            startGamePermissionManager.setNotStartGamePermitted(p);
            playerManager.sendCommandTree(p);
        });

        return CompletableFuture.completedFuture(Optional.of(new CommandFeedback(Text.literal("Players denied."), false, false)));
    }

    static void register(LiteralArgumentBuilder<ServerCommandSource> lab) {
        lab.then(literal("denyGameStart")
                .requires(s -> s.hasPermissionLevel(4))
                .then(argument("players", players())
                        .executes(new DenyGameStartCommand())));
    }
}
